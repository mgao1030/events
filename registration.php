...
if (isset($_POST['submit'])) { //check if the form has been submitted
	if ((empty($_POST['num_guests'])) || (empty($_POST['mealpref_id'])) ) {
		...
		$query = "INSERT INTO registration VALUES(NULL, \"$user_id\", \"$event_id\", \"$date_registration\", \"$mealpref_id\", \"$num_guests\")";
		$result = $conn->query($query);
		if (!$result) {
			die ("Database access failed: " . $conn->error);
		} else {
			$message = "<p class=\"message\">Thank you for your interest. You have successfully registered the event! <br><a href=\"index.php\"><br/>Return to events list</a></p>";
		}
	}
}
?>
